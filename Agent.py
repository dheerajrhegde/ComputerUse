...
st.title('Computer Use using LLM')

# Text area to input prompt
...
if st.button("Submit"):
    if prompt:
        try:
            ...
            abot = Agent(model, tool, "You are an automation assistant for the user. Complete the tasks using the tools provided.")
            response = abot.graph.invoke({"messages": [prompt]})
            #st.write("Response:")
            #st.write(response)
